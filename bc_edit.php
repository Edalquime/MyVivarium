...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // 🐁 MODIFICACIÓN PARA MACHOS REPRODUCTORES (Y PERMITIR EL 0)
            $cant_machos = (isset($_POST['cant_machos']) && $_POST['cant_machos'] !== '') ? intval($_POST['cant_machos']) : 0;

            // Suponiendo que tu tabla "holding" posee la columna "cant_machos" (puedes adaptarlo según tu base de datos)
            ...
            $stmtCages->bind_param("sss", $pi_name, $remarks, $cage_id); // Modificado a string para evitar errores con mysqli_real_escape_string
            $stmtCages->execute();
            $stmtCages->close();
            ...
            for ($i = 0; $i < count($mouse_ids); $i++) {
                ...
                if ($mouse_id !== '') { // Cambiado de !empty a comparación directa para que tome el "0"
                        ...
                        <div class="section-card">
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    <label for="cant_machos" class="form-label">Cantidad de Machos <span class="required-asterisk">*</span></label>
                                    <input type="number" class="form-control" id="cant_machos" name="cant_machos" value="<?= isset($holdingcage['cant_machos']) ? intval($holdingcage['cant_machos']) : 0; ?>" min="0" required>
                                </div>
                            </div>
                        </div>
